        ...
        <div className="flex-1">
          ...
          <h2
            className={clsx(
              ...
              featured
                ? "text-2xl sm:text-3xl lg:text-[2.55rem]"
                : "text-xl sm:text-2xl lg:text-[2rem]",
            )}
          >
            <Link
              href={`/blog/${post.slug}`}
              className="focus-ring inline-block py-1 transition-colors hover:text-(--accent)"
            >
              {post.title}
            </Link>
          </h2>
          ...
          {post.description ? (
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-(--foreground-soft) sm:text-base lg:mt-4 lg:max-w-3xl lg:text-[1.08rem] lg:leading-8">
              {post.description}
            </p>
          ) : null}
