...
export function PostPageHeader({ post }: PostPageHeaderProps) {
  ...
  return (
    <header className="mb-10 sm:mb-12 lg:mb-14">
      ...
      <h1
        className="mb-5 text-3xl leading-tight text-(--foreground) sm:mb-6 sm:text-4xl md:text-5xl lg:text-[4rem]"
        style={{ fontWeight: 600 }}
      >
        {post.title}
      </h1>
      {post.description ? (
        <p className="max-w-2xl text-base leading-relaxed text-(--foreground-soft) sm:text-lg lg:max-w-3xl lg:text-[1.24rem] lg:leading-9">
          {post.description}
        </p>
      ) : null}
      ...
      <div className="mt-6 flex flex-wrap gap-2 text-xs lg:mt-7 lg:gap-2.5 lg:text-[0.82rem]">
        {post.tags.map((tag) => (
          <span
            key={tag}
            className="inline-flex items-center gap-1 rounded-full border border-(--border) px-2.5 py-1 text-(--muted) lg:px-3 lg:py-1.5"
          >
